...
const Cart = ({ location }) => {
  ...
  const makePayment=()=>
{
getData({amount:dataReducer1.grandTotal.toFixed(2)})
}
...
const getData=(data)=>
{
  ...
  .then((res) => {
    ...
    // console.log("iampaytm",JSON.stringify(paytm),information)
  ...
  // console.log("CartItems", dataReducer.cartItems1);
  const check = () => {
    if (checked == false) {
if(dataReducer1.grandTotal>1000)
{
  setChecked(!checked)
  dispatch(fetchcartprice(disco,addToast))
}
else{
      ...
      if (vouchers[i].voucherId === discount.toString().toLowerCase()) {
        if(discount.toString().toLowerCase()==="nikhilmahajan20")
        {
          if(dataReducer1.grandTotal>=500)
          {
            ...
            setDisco(disc);
            dispatch(fetchcartprice(disc,addToast));
            // if(messageReferral!="")
            // {
            //   addToast('Only voucher or referral code can be applied at a time', {
            //     appearance: 'warning',
            ...
            dispatch(fetchcartprice(0,addToast));
        ...
        {
          if(dataReducer1.grandTotal>=1500)
          {
            disc = vouchers[i].discount;
            setDisco(disc);
            dispatch(fetchcartprice(disc,addToast));
        ...
        .then((response) => {
          ...
          if (response.data.success == true) {
            ...
            if (available == true) {
              ...
              return fetchApi('/userdash/orderNow', obj, {}, true, 'post')
                .then((response) => {
                  // console.log("Response", response.data);
                  if (response.data.message === 'success') {
        ...
        .then((response) => {
          ...
          if (response.data.message === 'success') {
            let obj = {
              ...
              useCredits:checked,
              mobile:mobilenumber,
              billing_address: billerror==true? {area:address2.area2,
                city:address2.city2,
              state:address2.state2,
            firstName:address2.firstName2,
            pincode:address2.pincode2,
          lastName:address2.lastName2}:address,
              email: localStorage.getItem("useremail"),
              total: dataReducer1.grandTotal,
              userId: localStorage.getItem("userId"),
              orderType:"order"
            };
            localStorage.setItem("order",JSON.stringify(obj))
  ...
  useEffect(() => {
    fetchcartprice(0,addToast)
    dispatch(fetchcarts(addToast, 0,false));
    init12();
    init202();
    init2();
    init3();
                    ...
                    <div className='cart-shiping-update-wrapper'>
                      <div className="row justify-content-between">
                        ...
                        <div className="col-md-5">
                          ...
                          <div style={{marginLeft:"140px"}} className="row px-8">
                        ...
                        <div className='discount-code-wrapper'>
                          ...
                          <div className='discount-code'>
                           ...
                           {vouchershow===false?(<span>{discount} applied  {message.split("Applied")[1]}
                            <button onClick={()=>{setVoucherShow(true)
                             dispatch(fetchcartprice(0,addToast))}} style={{ backgroundColor: 'rgb(0, 41, 95)', color: '#fff',position: "absolute",
    left: "309px",
    bottom: "48px",
    width:"18%" }} className='cart-btn-2 mt-10' type='submit'>
                                Remove
                          </button>
                              ...
                              />
                              <button style={{ backgroundColor: 'rgb(0, 41, 95)', color: '#fff' }} className='cart-btn-2 mt-10' type='submit'>
                                Apply Coupon
                          </button>
                            </form>)} 
                            {/* <p>{message}</p> */}
                  ...
                  <div className='col-lg-4 col-md-12'>
                    <div className='grand-totall'>
                      ...
                      <h4 className='grand-totall-title'>
                        Sub Total<span>₹{dataReducer1 != undefined ? (parseFloat(dataReducer1.total + dataReducer1.totaldiscount).toFixed(2)) : null}</span>
                      </h4>
                      {dataReducer1 != undefined && dataReducer1.totaldiscount > 0||dataReducer1.credit>0 ? (<h4 className='grand-totall-title'>
                        Total Discount{' '}
                        <span>₹{dataReducer1 != undefined ? (parseFloat(dataReducer1.totaldiscount+dataReducer1.credit).toFixed(2)) : null}</span>
                      ...
                      {dataReducer1 != undefined &&dataReducer1.credit>0 ? (<h4 className='grand-totall-title'>
                        Credits {' '}
                        <span>₹{dataReducer1 != undefined ? (dataReducer1.credit.toFixed(2)) : null}</span>
                        
                      </h4>) : null}
                      {/* {dataReducer1 != undefined && dataReducer1.totaldiscount > 0 ? (<h4 className='grand-totall-title'>
                       {vouchershow==false?("Voucher"):("Referral")} 
                        <span>₹{dataReducer1 != undefined ? (dataReducer1.totaldiscount.toFixed(2)) : null}</span>
                        
                      </h4>) : null} */}
                      {/* <h4 className='grand-totall-title'>
                        Total Tax{' '}
                        <span>₹{dataReducer1 != undefined ? (dataReducer1.totaltax.toFixed(2)) : null}</span>
                      </h4> */}
                   
                      <h4 className='grand-totall-title'>
                        Grand Total{' '}
                        <span>₹{dataReducer1 != undefined ? (dataReducer1.grandTotal.toFixed(2)) : null}</span>
                      </h4>
